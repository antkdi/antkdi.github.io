...
제2절 DDL ( DATA DEFENITION LANGUAGE )
 ...
 2. 제약조건
  1) PRIMARY KEY : 테이블에 저장된 행 데이터를 고유하게 식별하기 위한 기본 키를 정의한다.
  ...
  2) UNIQUE KEY : 테이블에 저장된 행 데이터를 고유하게 식별하기 위한 기본키를 정의한다. 단, NULL 은 고유키 제약 대상
          이 아니므로 NULL 값을 가진 행이 여러개가 있더라도 고유키 제약위반이 되지 않는다.
  
  3) NOT NULL : NULL 값의 입력을 허용하지 않는다. 디폴트 상태에서는 모든 칼럼에서 NULL 을 허용하지만 , 이 제약조건
         을 지정함으로써 해당 칼럼은 입력 필수가 된다. NOT NULL 을 CHECK 의 일부분 으로 이해할 수 있다.
  
  4) CHECK : 입력할 수 있는 값의 범위등을 제한 . CHECK 제약으로는 TRUE OR FALSE 로 평가 할수 있는 논리식을 지정
  
  5) FOREIGN KEY : 관계형 데이터베이스 에서 테이블 간의 관계를 정의하기 위해 기본키를 다른 테이블의 외래키로 복사하는
